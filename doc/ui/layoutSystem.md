...
The padding parameter also allows several shortcuts for common use cases.

1. To apply the same value to all sides, simply add only one unamed value to the table or simpler only add a value.<br>
   **Example**: `padding={2}` or `padding=1`
2. To apply the same value to top and bottom (first value) and left to right (second value),
   only add two values to the table<br>
   **Example**: `padding={1,2}`

If you dont want to apply padding to all sides prefer using named parameter to prevent misunderstanding.

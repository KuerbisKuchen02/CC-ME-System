...
| FR-XFN-04 | The build system MUST distinguish static and dynamic dependencies.                                                                        | US-DEP-07                       |            |
| FR-XFN-05 | Application and plugin versions MUST NOT replace repository source revisions as the mechanism for reproducing an exact source state.      | US-DEP-13, US-DEP-14, US-DEP-15 |            |
| FR-XFN-06 | Each project or library MUST define its build configuration in a build configuration file located at the root of that project or library. | US-DEP-02                       |            |
| FR-XFN-07 | The build system diagnostics MUST clearly identify the diagnostic category, affected project/target/module, and cause.                    | US-DEP-18                       |            |
| FR-XFN-08 | Each build system component/ stage MUST use consistent naming conventions and use unique diagnostic codes.                                | US-DEP-18                       |            |
| FR-XFN-09 | A build performed from a specific repository revision MUST resolve in-repository dependencies from that same revision.                    | US-DEP-15                       |            |
...
| ID         | Requirement                                                                                                                 | Reference | Dependency |
| ---------- | --------------------------------------------------------------------------------------------------------------------------- | --------- | ---------- |
| FR-DEP-005 | The build system MUST report a warning when a dependency is declared but is not used by the relevant source tree.           | US-DEP-03 |            |
| FR-DEP-006 | The build system MUST report an error when statically resolvable source code uses a dependency that has not been declared.  | US-DEP-04 |            |
| FR-DEP-007 | Dependency diagnostics MUST identify the affected dependency and the source/configuration location relevant to the problem. | US-DEP-18 |            |
| FR-DEP-008 | Dependency diagnostics SHOULD provide actionable information describing how the problem can be corrected.                   | US-DEP-18 |            |
| FR-DEP-009 | Dependency diagnostics MUST be distinguishable from build, bundling, minification, deployment, and other error categories.  | US-DEP-18 |            |
...
| FR-DEP-302 | A dependency reached through multiple dependency paths MUST be resolved and included only once in a resulting artifact.                                                                                                         | US-DEP-17            |            |
...
| FR-DEP-306 | Library internal implementation modules SHOULD NOT be treated as part of the library's public API unless explicitly exposed by the library.                                                                                     | US-DEP-16            |            |
| FR-DEP-307 | Static dependencies MUST be eligible for dependency resolution and bundling according to the selected build type.                                                                                                               | US-DEP-07            |            |
| FR-DEP-308 | Dynamic dependencies MUST NOT be statically resolved or bundled into the referencing artifact.                                                                                                                                  | US-DEP-08            |            |
| FR-DEP-309 | A `require()` whose argument is a string literal MUST be treated as statically resolvable, while a `require()` whose argument is not a string literal MUST be treated as dynamic.                                               | US-DEP-07, US-DEP-09 |            |
| FR-DEP-310 | When multiple modules require the same static dependency, the bundler MUST include the dependency only once in a bundled artifact.                                                                                              | US-DEP-17            |            |
...
| FR-DEP-504 | The development tooling SHOULD expose the build system's module mappings to Lua Language Server. If Lua Language Server cannot resolve the mappings through generated/configured workspace information, source annotations MAY be used as a fallback. | US-BLD-18 |            |
...
| FR-DEP-602 | For project modules, the default logical module name MUST be derived from the path below the project's `src/` directory by replacing path separators with `.` and removing the `.lua` extension. | US-BLD-03 |            |
| FR-DEP-603 | For shared-library modules, the default logical module name MUST be derived from the configured library source root and prefixed with `lib.`.                                                    | US-BLD-04 |            |
| FR-DEP-604 | For external modules, the default logical module name MUST be derived from the configured external source root and prefixed with `external.`.                                                    | US-BLD-04 |            |
...
| FR-BLD-205 | Development builds MUST NOT minify application source code by default.                                                                          | US-BLD-14 |            |
| FR-BLD-206 | Test builds MUST use release-like bundling and minification behavior by default.                                                                | US-BLD-25 |            |
| FR-BLD-207 | Release builds MUST use release-oriented bundling and minification behavior by default.                                                         | US-BLD-15 |            |

## 4.3 Bundling and Minification

| ID         | Requirement                                                                                                                                                              | Reference | Dependency |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- | ---------- |
| FR-BLD-301 | The default bundling behavior MUST bundle the target entry point and its statically resolved bundled dependencies into a single main program where technically possible. | US-BLD-15 |            |
| FR-BLD-303 | Release builds MUST minify bundled source code.                                                                                                                          | US-BLD-15 |            |
| FR-BLD-304 | Release builds SHOULD remove comments and other unnecessary source representation where doing so reduces artifact size without changing program behavior.                | US-BLD-15 |            |
| FR-BLD-305 | The build system MUST preserve the functional distinction between bundled source and files explicitly excluded from bundling.                                            | US-BLD-15 |            |
| FR-BLD-306 | When a module is bundled into the main artifact, its individual `require()` statement MUST be resolved at build time and replaced with the source code of the module     | US-BLD-15 |            |
...
| ID         | Requirement                                                                                                   | Reference | Dependency |
| ---------- | ------------------------------------------------------------------------------------------------------------- | --------- | ---------- |
| FR-BLD-033 | The build process MUST consist of clearly defined processing stages.                                          | US-BLD-19 |            |
| FR-BLD-034 | Dependency resolution MUST occur before dependency-dependent bundling decisions are made.                     | US-BLD-19 |            |
| FR-BLD-035 | Dependency validation MUST occur before producing an artifact that depends on the validated dependency graph. | US-BLD-19 |            |
| FR-BLD-036 | Bundling MUST operate only on components selected for inclusion in the artifact.                              | US-BLD-19 |            |
| FR-BLD-037 | Minification MUST operate only on source selected for minification.                                           | US-BLD-19 |            |
| FR-BLD-038 | Artifact creation MUST combine the generated bundled output with applicable preserved/generated files.        | US-BLD-19 |            |
...
| FR-TST-604 | Test artifacts MUST preserve the complete module structure and MUST NOT be bundled or minified so that source file names and line numbers remain meaningful. | US-BLD-25 |            |

## 5.7 Test Quality Attributes

| ID          | Requirement                                                                                                                                                   | Reference            | Dependency |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | ---------- |
| NFR-TST-701 | Test suites MUST be isolated from one another through fresh suite fixture instances.                                                                          | US-TST-04            |            |
| NFR-TST-702 | The test framework MUST NOT be responsible for isolating system resources such as the filesystem. Such isolation is the responsibility of the test developer. | US-TST-20            |            |
| NFR-TST-703 | The test framework MUST provide deterministic execution by default and MUST provide sufficient information to reproduce randomized execution.                 | US-TST-07, US-TST-09 |            |

-- Modified to use errorManager.error() instead of error()
...
local function expect(index, value, ...)
    ...
    if name then
        errorManager.error(("bad argument #%d to '%s' (%s expected, got %s)"):format(index, name, type_names, t), 3)
    else
        errorManager.error(("bad argument #%d (%s expected, got %s)"):format(index, type_names, t), 3)
    end
end
...
local function field(tbl, index, ...)
    ...
    if value == nil then
        errorManager.error(("field '%s' missing from table"):format(index), 3)
    else
        errorManager.error(("bad field '%s' (%s expected, got %s)"):format(index, get_type_names(...), t), 3)
    end
end
...
local function range(num, min, max)
  ...
  if min > max then
    errorManager.error("min must be less than or equal to max)", 2)
  end

  if is_nan(num) or num < min or num > max then
    errorManager.error(("number outside of range (expected %s to be within %s and %s)"):format(num, min, max), 3)
  end
